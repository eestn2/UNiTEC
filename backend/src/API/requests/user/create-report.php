...
if ( !isset($data->reported_id) || !isset($data->reason)) return_response_outdated("failed", "Faltan datos.", null);
...
try{
    try {
        ...
        $connection->commit();
        return_response_outdated("success", "Reporte enviado con exito.", null);
    } catch (PDOException $e) {
        if ($connection->inTransaction()) {
            $connection->rollBack();
        }
        return_response_outdated("failed", "Error al insertar el reporte: " . $e->getMessage(), null);
    }
}catch (PDOException $e) {
    return_response_outdated("failed", "Error al reportar usuario:" . $e->getMessage(), null);
}
?>