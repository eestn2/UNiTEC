...
if (!isset($data->reviewed_id) || !isset($data->text)) return_response_outdated("failed", "Faltan datos.", null);
...
try{
    try {
        ...
        $connection->commit();
        return_response_outdated("success", "Resenia enviado con exito.", null);
    } catch (PDOException $e) {
        if ($connection->inTransaction()) {
            $connection->rollBack();
        }
        return_response_outdated("failed", "Error al insertar la resenia: " . $e->getMessage(), null);
    }
}catch (PDOException $e) {
    return_response_outdated("failed", "Error al reseniar usuario:" . $e->getMessage(), null);
}
?>