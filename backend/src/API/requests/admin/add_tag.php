...
if (!isset($data->name) || empty(trim($data -> name))) return_response_outdated("failed", "Faltan datos.", null);

if (!isset($_SESSION['user']['id'])) return_response_outdated("failed", "No autenticado.", null);
if (!is_admin($_SESSION['user']['id'], $connection)) {
    return_response_outdated("failed", "Solo los administradores pueden agregar tags.", null);
}
$name = $data->name;
try{
    ...
    $connection->commit();
    return_response_outdated("success", "tag insertada con exito.", null);
}catch(PDOException $e) {
    return_response_outdated("failed","Error al insertar la tag". $e->getMessage(), null);
}
?>