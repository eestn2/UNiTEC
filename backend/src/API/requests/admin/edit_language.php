...
if (!isset($data->id) || !isset($data->name) || empty(trim($data -> name))) return_response_outdated("failed", "Faltan datos.", null);

if (!isset($_SESSION['user']['id'])) return_response_outdated("failed", "No autenticado.", null);
if (!is_admin($_SESSION['user']['id'], $connection)) {
    return_response_outdated("failed", "Solo los administradores pueden cambiar lenguajes.", null);
}
...
try{
    ...
    $stmt->execute();
    $connection->commit();
    return_response_outdated("success", "lenguaje editado con exito.", null);

}catch(PDOException $e) {
    return_response_outdated("failed","Error al editar el lenguaje". $e->getMessage(), null);
}
?>