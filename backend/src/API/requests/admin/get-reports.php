<?php
session_start();

require_once __DIR__ . "/../cors-policy.php";
require_once __DIR__ . '/../../logic/database/connection.php';
require_once __DIR__ . '/../../logic/communications/return_response.php';
require_once __DIR__ . '/../../logic/security/is_admin.php';

if ($_SERVER["REQUEST_METHOD"] !== "GET") return_response_outdated("failed", "Metodo no permitido.", null);

if (!isset($_SESSION['user']['id'])) return_response_outdated("failed", "No autenticado.", null);
if (!is_admin($_SESSION['user']['id'], $connection)) {
    return_response_outdated("failed", "Solo los administradores pueden ver los reportes.", null);
}

try {
    $stmt = $connection->prepare("
        SELECT r.*, 
            reporter.email AS reporter_email, 
            reported.email AS reported_email
        FROM reports r
        JOIN users reporter ON r.reporter_id = reporter.id
        JOIN users reported ON r.reported_id = reported.id
    ");
    $stmt->execute();
    $reports = $stmt->fetchAll(PDO::FETCH_ASSOC);
    return_response_outdated("success", "reports retrieved successfully.", ["reports" => $reports]);
} catch (PDOException $e) {
    error_log("Error retrieving reports: " . $e->getMessage());
    return_response_outdated("failed", "Error retrieving reports.", null);
}

?>